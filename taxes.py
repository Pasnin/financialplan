...
if __name__ == '__main__':
    # Example usage
    example_income = 675_000  # NOK
    example_wealth = 755_906  # NOK
    example_mortgage = 3990000/2  # NOK
    example_loans = 425_000  # NOK
    example_bank_balance = 200_000  # NOK
    
    # Customize interest rates if desired
    mortgage_interest_rate = 0.0577  # 4.7% for mortgage
    other_loans_interest_rate = 0.045  # 5.8% for other loans
    
    tax_result = calculate_norwegian_tax(
        income=example_income, 
        wealth=example_wealth,
        mortgage=example_mortgage,
        loans=example_loans,
        bank_balance=example_bank_balance,
        income_type='wage', # 'wage', 'self_employment', or 'pension'
        year=2025,
        mortgage_interest_rate=mortgage_interest_rate,
        other_loans_interest_rate=other_loans_interest_rate
    )
    
    print(f"Tax Calculation for {tax_result['year']}")
    print(f"Income: {tax_result['income']:,} NOK")
    print(f"Income Type: {tax_result['income_type']}")
    print(f"Gross Wealth: {tax_result['gross_wealth']:,} NOK")
    print(f"Bank Balance: {tax_result['bank_balance']:,} NOK")
    print(f"Mortgage: {tax_result['mortgage']:,} NOK (rate: {tax_result['interest']['mortgage_interest_rate']:.2%})")
    print(f"Other Loans: {tax_result['loans']:,} NOK (rate: {tax_result['interest']['other_loans_interest_rate']:.2%})")
    print(f"Net Wealth for Tax: {tax_result['net_wealth_for_tax']:,} NOK")
    
    print("\nInterest Payments:")
    print(f"  Mortgage Interest: {tax_result['interest']['mortgage_interest']:,.2f} NOK per year")
    print(f"  Other Loans Interest: {tax_result['interest']['other_loans_interest']:,.2f} NOK per year")
    print(f"  Total Interest: {tax_result['interest']['total_interest']:,.2f} NOK per year")
    
    print("\nTax Components:")
    for component, amount in tax_result['tax_components'].items():
        print(f"  {component.replace('_', ' ').title()}: {amount:,.2f} NOK")
    
    print(f"\nTotal Tax: {tax_result['total_tax']:,.2f} NOK")
    print(f"Effective Tax Rate: {tax_result['effective_tax_rate']:.2%}")
    
    # Show interest deduction benefit
    interest_deduction_value = -tax_result['tax_components']['interest_deduction']
    print(f"\nInterest Deduction Benefit: {interest_deduction_value:,.2f} NOK (tax savings)")
